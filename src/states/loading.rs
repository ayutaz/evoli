...
fn load_wind_config() -> Wind {
    match std::fs::read_to_string("resources/wind.ron") {
        Ok(contents) => match ron::de::from_str::<Wind>(&contents) {
            Ok(wind) => wind,
            Err(e) => {
...
fn load_creature_prefabs() -> CreaturePrefabs {
    ...
    let creature_files = [
        ...
        (
            "HerbivoreCarcass",
            "resources/prefabs/creatures/herbivore_carcass.ron",
        ),
    ...
    for (key, path) in &creature_files {
        match std::fs::read_to_string(path) {
            Ok(contents) => match ron_options.from_str::<CreatureDefinition>(&contents) {
                Ok(def) => {
